...
# REL-3: regenerate derived content (index.html, feeds, optimized images) BEFORE
# rsync, so running deploy.sh standalone never publishes a stale index/feed.
#   - We invoke build.sh WITHOUT --deploy and with JLW_SKIP_DEPLOY_CHAIN=1 so it
#     regenerates but does NOT chain back to deploy.sh (no build->deploy->build loop).
#   - If we were ourselves invoked via `build.sh --deploy`, JLW_ALREADY_BUILT is
#     already set, so we skip this rebuild. Net effect: exactly one regeneration
#     per publish, with no recursion.
if [[ -n "${JLW_ALREADY_BUILT:-}" ]]; then
    echo "ℹ  Content already regenerated by build.sh — skipping rebuild."
else
    echo "🏗  Regenerating derived content before deploy (build.sh) ..."
    JLW_SKIP_DEPLOY_CHAIN=1 "$SCRIPT_DIR/build.sh" "$DEPLOY_SOURCE"
    echo ""
fi
